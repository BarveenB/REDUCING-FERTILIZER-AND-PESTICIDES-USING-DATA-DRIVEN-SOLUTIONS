...
# Load pest detection model if TensorFlow is available
...
ferti = {
    ...
    'Paddy Stem Maggot': {
        'en': 'Regular application of a balanced fertilizer with adequate amounts of phosphorus and potassium can help prevent brown spot disease in plants.',
        'ta': 'சமநிலை போஸ்பரஸ் மற்றும் பொட்டாசியம் கொண்ட உரம் முறையாக பயன்படுத்துவதன் மூலம் செடிகளில் பழுப்பு கற்கள் நோயைத் தடுப்பது சாத்தியமாகும்.'
    },
    # Add more pest recommendations as needed
}
...
- Pest detection from plant images
- Agricultural advice through chatbot
""")

# Navigation - only show pest detection if available
...
if page == "Crop Recommendation":
    ...
    if crop_model is None or label_encoders is None:
        st.error("Crop recommendation service is currently unavailable")
    else:
        ...
        if submit_button:
            try:
                ...
                # Text-to-speech for recommendation
                try:
                    tts = gTTS(text=f"The recommended crop is {crop}", lang='en')
                    tts.save("temp/crop_recommendation.mp3")
                    audio_file = open("temp/crop_recommendation.mp3", "rb")
...
# Pest Detection Page
    ...
    if pest_model is None:
        st.error("Pest detection service is currently unavailable")
    else:
                ...
                if st.button('Detect Pest'):
                    try:
                        ...
                        if index < len(pest_classes):
                            ...
                            st.error("Could not identify the pest from the image")
                    except Exception as e:
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "Keep responses focused on agriculture and provide practical advice. "
                    "When asked about specific problems, offer detailed solutions including organic and "
                    "chemical options if appropriate."
                )
            },
